import React from 'react';
import BooksTable from '@/app/components/dashboard/BooksTable';
import { getBooks } from '@/lib/api/books';

const ManageEbooksPage = async () => {
  // Fetch global ebook repository array collection
  const books = await getBooks();

  return (
    <div className="min-h-screen bg-[#121212] text-white w-full px-4 sm:px-6 md:px-8 py-8 font-sans">
      <div className="w-full mx-auto max-w-[100%]">
        
        {/* Dynamic Context Header block */}
        <div className="flex flex-row justify-between items-center mb-6 px-1">
          <h2 className="text-xl sm:text-2xl font-semibold tracking-tight text-zinc-100">
            Manage All Platform Ebooks
          </h2>
          <span className="text-xs font-semibold text-zinc-400 bg-zinc-800/60 border border-zinc-800 px-3 py-1.5 rounded-full">
            Total Database Items: {books?.length || 0}
          </span>
        </div>

        {/* Full-Width Responsive Table Structure */}
        <div className="w-full bg-[#1c1c1e] border border-zinc-800/80 rounded-xl shadow-2xl overflow-hidden">
          <div className="w-full overflow-x-auto scrollbar-thin scrollbar-thumb-zinc-800 scrollbar-track-transparent">
            <table className="w-full min-w-[850px] text-left border-collapse table-auto">
              <thead>
                <tr className="border-b border-zinc-800/60 text-zinc-400 text-xs sm:text-sm font-medium tracking-wide bg-zinc-900/20">
                  <th className="py-5 px-4 sm:px-6 select-none">Ebook Title</th>
                  <th className="py-5 px-4 sm:px-6 select-none">Writer Name</th>
                  <th className="py-5 px-4 sm:px-6 select-none">Price</th>
                  <th className="py-5 px-4 sm:px-6 select-none">Status</th>
                  <th className="py-5 px-4 sm:px-6 text-right select-none">Administrative Tools</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-zinc-800/40">
                {books && books.length > 0 ? (
                  books.map((book, index) => (
                    <BooksTable 
                      key={book._id} 
                      book={book} 
                      index={index}
                    />
                  ))
                ) : (
                  <tr>
                    <td colSpan="5" className="py-12 text-center text-sm text-zinc-500">
                      No book listings detected inside global document database collections.
                    </td>
                  </tr>
                )}
              </tbody>
            </table>
          </div>
        </div>

      </div>
    </div>
  );
};

export default ManageEbooksPage;